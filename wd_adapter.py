...
class BaseWatchDataBlockchainService:
    ...
    @retry(tries=3)
    def _raw_request(self, method: str, endpoint: str, params: Optional[dict] = None, json: Optional[dict] = None):
    ...
    def add_filter(self, address: str, contract_address: Optional[str],  with_approval: bool = False) -> str:
        request_json = {'address': address, 'contract_address': contract_address, 'with_approval': with_approval}
    ...
    def check_filters(self, block_number: int):
        raw_response = self._raw_request('post', 'watch_filters/check', json={'block_number': block_number})
        print(raw_response)
        return raw_response['filters']

    def check_eth_balance(self, address: str):
        raw_response = self._raw_request('post', f'node/jsonrpc?api_key={WATCH_API_KEY}', json={
            "jsonrpc": "2.0",
